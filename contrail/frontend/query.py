...
def list_instances(**kwargs) -> List:
    ...
    :param kwargs: A set of filters for searching for instances.
...
def get_instance_price_history(**kwargs) -> List[Dict]:
    """
    Get a set of price history points for a given instance described by `kwargs`.

    :param kwargs: A set of filters used to identify the desired instance. Must at least consist of the fields specified
                   by this provider's DISCRIMINATORS.

    :return: A List of "history points", where each history point is a dictionary consisting of crawlTime, priceType,
             and optionally pricePerHour, priceUpfront, and leaseContractLength.
    """
    ...
    price_history = InstanceData.objects_in(db)\
                        .filter(**kwargs).distinct().only(*PRICE_HISTORY_PARAMS).order_by('-crawlTime')[:100]
